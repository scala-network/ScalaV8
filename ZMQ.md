# The Current/Future Status of ZMQ in Scala

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. Scala
...
matching supported by both Scala and ZMQ. The `format`, `context` and `event`
...
The Scala daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The Scala daemon also ensures that
...
The Scala daemon will send a `txpool_add` pub exactly once for each
