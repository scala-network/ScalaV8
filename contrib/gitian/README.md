...
```bash
git clone git@github.com:scala-project/gitian.sigs.git
...
gpg --detach-sign ${VERSION}-linux/${GH_USER}/scala-linux-*-build.assert
gpg --detach-sign ${VERSION}-win/${GH_USER}/scala-win-*-build.assert
gpg --detach-sign ${VERSION}-osx/${GH_USER}/scala-osx-*-build.assert
gpg --detach-sign ${VERSION}-android/${GH_USER}/scala-android-*-build.assert
```
