...
if test -z "$scalad"
then
  echo "scalad not found"
  exit 1
fi
echo "Found: $scalad"

TORDIR="$DIR/scala-over-tor"
TORRC="$TORDIR/torrc"
...
"$scalad" \
  ...
  --add-priority-node 2xlanode5itf65lz.onion:18083 \
  --detach
ready=0
for i in `seq 10`
do
  sleep 1
  status=$("$scalad" status)
...
then
  ...
  tail -n 400 "$HOME/.bitscala/bitscala.log" | grep -Ev stacktrace\|"Error: Couldn't connect to daemon:"\|"src/daemon/main.cpp:.*Monero\ \'" | tail -n 20
  exit 1
fi
