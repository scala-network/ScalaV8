...
def difficulty():
    ...
    while True:
        if len(times) <= 1:
            ...
            begin = max(len(times) - DIFFICULTY_WINDOW_NEW - DIFFICULTY_LAG, 0)
            end = min(begin + DIFFICULTY_WINDOW_NEW, len(times))
            length = end - begin
            assert length >= 2
            if length <= DIFFICULTY_WINDOW_NEW - 2 * DIFFICULTY_CUT:
                cut_begin = 0
                cut_end = length
            else:
                excess = length - (DIFFICULTY_WINDOW_NEW - 2 * DIFFICULTY_CUT)
